  ...
  return (
    <Select onValueChange={handleChange}>
      <SelectTrigger
        className={`
          w-full text-primary 
          border-transparent border-b-primary rounded-none
          focus:border-transparent focus:border-b-white 
          data-[placeholder]:text-primary
          [&_svg:not([class*='text-'])]:text-primary cursor-pointer ${className}
        `}
      >
      ...
      <SelectContent className="bg-secondary text-foreground border-none">
        {options.map((item, index) => (
          <SelectItem
            ...
            className="bg-none hover:bg-primary"
          >
            {item}
          </SelectItem>
        ))}
